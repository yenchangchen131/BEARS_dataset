    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if "rate_limit" in error_msg.lower() or "429" in error_msg:
                sleep_time = (2**attempt) * 2  # Rate limit 需要等久一點
                print(f"  [Warning] Rate limit hit, retrying in {sleep_time}s...")
            else:
            ...
            if attempt < MAX_RETRIES - 1:
                ...
                print(f"  [Error] 翻譯失敗: {error_msg[:100]}...")
                return text
...
def process_item(item: dict, fields: list[str]) -> dict:
    ...
    # 檢查是否需翻譯
    source = translated_item.get("source_dataset") or translated_item.get(
        "original_source", ""
    )
    if source == "drcd":
...
def translate_batch_parallel(
    items: list[dict], fields: list[str], desc: str
) -> list[dict]:
    """多執行緒並行翻譯"""
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for future in tqdm(as_completed(future_to_index), total=len(items), desc=desc):
            ...
            try:
                ...
                results[index] = items[index]  # 發生錯誤時保留原始資料
...
def main():
    ...
    translated_queries = translate_batch_parallel(
        queries_raw, ["question", "gold_answer"], "翻譯問答"
    )

    # 翻譯文檔
    print("\n[翻譯文檔資料]")
    translated_corpus = translate_batch_parallel(corpus_raw, ["content"], "翻譯文檔")
