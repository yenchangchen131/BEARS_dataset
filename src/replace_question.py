...
def main():
    if len(sys.argv) < 2:
        ...
        print(
            "範例: uv run src/replace_question.py e7124c20-75c2-5d99-9acf-d2cba40228fa"
        )
        sys.exit(1)
    ...
    # 更新 corpus
    print("[6/6] 更新 corpus.json 與 corpus_raw.json...")
    ...
    for doc in corpus:
        if doc["doc_id"] in old_gold_doc_ids:
            old_doc_sources.add(
                doc.get("original_id", "").split("_")[0]
                if "_" in doc.get("original_id", "")
                else doc.get("original_id", "")
            )
    ...
    if source_dataset in datasets_with_hard_negs:
        ...
        if old_question_prefix:
            corpus = [
                ...
                for doc in corpus
                if not (
                    doc["original_source"] == source_dataset
                    and doc.get("original_id", "").startswith(old_question_prefix + "_")
                )
            ]
            corpus_raw = [
                doc
            ...
            corpus_raw = [
                doc for doc in corpus_raw if doc["doc_id"] not in old_gold_doc_ids
            ]
    else:
        corpus = [doc for doc in corpus if doc["doc_id"] not in old_gold_doc_ids]
        corpus_raw = [
